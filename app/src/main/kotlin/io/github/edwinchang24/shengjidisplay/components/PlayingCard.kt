...
@Composable
fun PlayingCard(card: PlayingCard, modifier: Modifier = Modifier, textStyle: TextStyle = LocalTextStyle.current) {
...
private fun PlayingCardPreview() {
    ShengJiDisplayTheme {
        Surface {
            PlayingCard(
                card = PlayingCard("A", Suit.SPADES), textStyle = LocalTextStyle.current.copy(fontSize = 128.sp)
            )
        }
    }
}
